...
// PointsMaterial, 
BufferGeometry, Float32BufferAttribute, LineSegments, LineBasicMaterial, InstancedMesh, DynamicDrawUsage, Color, Vector2, CircleGeometry, MeshBasicMaterial, PlaneGeometry, TextureLoader, DoubleSide } from 'three';
...
export class VectorTileRenderLayer extends OverlayLayer {
    ...
    processTileData(tile, data) {
                ...
                if (matchedPaintConfig) {
                    ...
                    if (type === 'LineString' || type === 'MultiLineString') {
                        ...
                        pointsList.forEach((pt) => {
                            ...
                            let scale = 1;
                            const size = matchedPaintConfig.size;
                            ...
                            if (size !== undefined) {
                                ...
                                if (matchedPaintConfig.type === 'icon') {
                                    // Icon Logic
                                    scale = (size * pixelsToUnit) / 10;
                                }
                                else {
                                    // Point Logic
                                    const radius = matchedPaintConfig.radius || 3;
                                    scale = radius / 5;
                                }
                            }
                            else {
                                // Default fallback
                                scale = matchedPaintConfig.radius ? (matchedPaintConfig.radius / 5) : 1;
                            }
                            const color = matchedPaintConfig.color ? new Color(matchedPaintConfig.color) : undefined;
                        ...
                        // }
                        const feature = this._createFeatureInstance(geometryToUse, rawFeature.geometry.type, matchedPaintConfig, rawFeature.properties);
                        if (feature) {
                            ...
                            if (feature instanceof Polygon && feature._renderObject) {
                                ...
                                feature._renderObject = undefined;
                            }
        ...
        lineGroups.forEach((group) => {
            ...
            const isTransparent = opacity < 1 || group.config.transparent === true;
            const width = group.config.width ?? 1;
            ...
            if (useWideLine) {
                ...
                if (!material) {
                    material = new LineMaterial({
                        color: group.config.color || 0xffffff,
                        linewidth: width,
                        transparent: isTransparent,
                        opacity: opacity,
                        dashed: Array.isArray(group.config.dashArray) && group.config.dashArray.length > 0,
                        dashScale: 1,
                        dashSize: group.config.dashArray?.[0] ?? 1,
                        gapSize: group.config.dashArray?.[1] ?? 0,
            ...
            mesh.position.copy(map.prjcenter);
        ...
        pointGroups.forEach((group) => {
            ...
            if (group.config.type === 'icon' && group.config.url) {
                // Geometry Cache
                ...
                if (!meshMaterial) {
                    ...
                    meshMaterial.onBeforeCompile = (shader) => {
                        shader.vertexShader = shader.vertexShader.replace('#include <project_vertex>', `
                            ...
                            `);
                        if (group.config.sizeAttenuation === false) {
                            ...
                            shader.vertexShader = shader.vertexShader.replace('// 3. Apply Vertex Offset in View Space (aligned with screen plane)', `
                                // 3. Apply Vertex Offset
                                
                                float attenuationFactor = 1.0;
                                bool isSizeAttenuation = ${group.config.sizeAttenuation !== false}; // Default true
            ...
            instancedMesh.position.copy(map.prjcenter);
