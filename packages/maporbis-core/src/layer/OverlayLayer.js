...
export class OverlayLayer extends Layer {
    ...
    addFeature(features) {
        ...
        for (const feature of featuresArray) {
            ...
            // debugger
            if (this._clouds) {
                this.map.sceneRenderer.scene.add(this._clouds);
    ...
    removeFeature(features) {
        ...
        for (let i = features.length - 1; i >= 0; i--) {
            ...
            if (!features[i] || this !== features[i].getLayer())
                continue;
            features[i]._remove();
        }
        return this;
     ...
     * @private
     */
    _disposeFeatureResources(feature) {
        const disposeLogic = () => {
            try {
                // Dispose geometry
                // 处理几何体
                ...
                if (feature instanceof Object3D) {
                    feature.traverse(child => {
                        if (child !== feature) { // Avoid duplicate processing 避免重复处理
                            this._disposeFeatureResources(child);
                        }
                    });
                }
            }
