...
export function getLocalInfoFromRay(map, ray) {
    ...
    const baseLayer = map.getLayers().find((layer) => layer.isBaseLayer === true);
    // 如果没有底图或者底图没有rootTile，返回undefined
    if (!baseLayer || !baseLayer._rootTile) {
        return undefined;
    }
    const baseRootTile = baseLayer._rootTile;
    const intersects = ray.intersectObjects([baseRootTile]);
    for (const intersect of intersects) {
        if (intersect.object instanceof Tile) {
            ...
            const localPoint = map._rootGroup.worldToLocal(worldPoint);
...
export function getLocalInfoFromGeo(map, geo) {
 ...
 * @returns 缩放级别
 */
export function getApproxZoomLevel(_camera, map) {
    return map.getZoom();
}
