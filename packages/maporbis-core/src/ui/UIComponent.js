...
export class UIComponent extends EventMixin(BaseMixin(EmptyBase)) {
    ...
    addTo(owner) {
        ...
        const map = typeof owner.getMap === "function" ? owner.getMap() : owner;
        if (!map) {
            return this;
        }
        this._map = map;
    ...
    show(coordinate) {
        ...
        // 记录显式传入的坐标（如果有）
        this._coordinate = coordinate ? [...coordinate] : undefined;
    ...
    _bindMapEvents(on) {
        ...
        const ensureHandler = (event, handler) => {
            if (on) {
        ...
        if (on) {
            ...
            const sceneRenderer = map.sceneRenderer;
    ...
    _resolveWorldPosition() {
        ...
        if (this._coordinate) {
            const [lng, lat, alt = 0] = this._coordinate;
        ...
        if (owner && owner._geometry) {
            ...
            if (g && (g.type === "Point" || g.type === "MultiPoint")) {
                ...
                if (g.type === "Point") {
                    ...
                    // MultiPoint：取第一个点
                    coordArr = g.coordinates[0];
                }
                if (coordArr && coordArr.length >= 2) {
                    const v = new Vector3(coordArr[0], coordArr[1], (coordArr[2] ?? 0));
                    return map.lngLatToWorld(v);
                }
            }
            ...
            if (owner._worldLngLatLikes instanceof Vector3) {
                const pos = owner._worldLngLatLikes;
