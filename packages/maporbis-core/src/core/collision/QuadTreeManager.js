...
export class QuadTreeManager {
     ...
     */
    _checkNodeCollision(box, x0, y0, x1, y1) {
    ...
    _getNodeData(node) {
        ...
        if (Array.isArray(node.data))
            return node.data; // 节点包含数据数组
        if (node.data)
            return [node.data]; // 节点包含单个数据对象
        return []; // 默认返回空数组
    }
    /**
   * 从四叉树中移除指定的边界框
