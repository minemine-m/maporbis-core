...
export class PerformanceMonitor {
     ...
     *                        附加统计信息
     */
    endFrame(frameId, additionalStats) {
     ...
     *                      执行时间（毫秒）
     */
    recordStrategyTime(strategyName, executionTime) {
     ...
     *                   检测次数
     */
    recordCollisionChecks(checkCount) {
        const frameStat = this.frameStats.get(this.currentFrameId);
        if (frameStat) {
    ...
    getStrategyPerformance(frames) {
        const strategyTimes = new Map();
    ...
    calculateTrends(frames) {
        if (frames.length < 2)
            return { frameTime: 'stable', fps: 'stable', features: 'stable' };
