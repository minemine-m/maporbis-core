...
import { CollisionReason } from './types/CollisionTypes';
...
export class CollisionEngine {
     ...
     */
    async update(camera) {
        if (!this._config.enabled || this._isUpdating)
            return;
     ...
     */
    _createCollisionContext(camera, timestamp) {
        return {
            camera,
    ...
    _collectCollidableFeatures() {
        const features = [];
        this._layers.forEach(layer => {
            const layerFeatures = layer.getFeatures().filter(f => f.collidable && f instanceof Feature);
     ...
     */
    async _applyCollisionResults(results, features) {
        const updates = features.map(feature => {
            const result = results.get(feature._id);
            if (!result)
                return Promise.resolve();
            feature.setCollisionVisibility(result.visible, result.reason);
            return Promise.resolve();
        });
