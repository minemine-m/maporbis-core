...
export class Polygon extends Surface {
    ...
    async _createObject(paint: Paint): Promise<Object3D> {
        const data = this._worldLngLatLikes;
        if (!data || data.length === 0) throw new Error("No geometry data");
        ...
        switch (paint.config.type) {
            ...
            case 'water':
                return _createWaterSurface(paint.config, this.getMap() as Map, finalData);
            case 'water-simple':
                return _createBaseWaterSurface(paint.config, finalData);
            case 'water-highlight':
                return _createHighlightWater(paint.config as any, this.getMap() as Map, finalData);
            default:
