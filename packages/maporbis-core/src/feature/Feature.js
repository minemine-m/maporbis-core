import { Object3D, Vector3, Mesh, Box3, Vector2 } from 'three';
...
import { CollisionType, CollisionReason } from '../core/collision/types/CollisionTypes';
...
export class Feature extends Handlerable(EventMixin(BaseMixin(Object3D))) {
    ...
    constructor(options) {
        ...
        this._paintManager = new FeaturePaintManager(() => this._renderObject, () => this._ensureRenderObjectInScene());
        this._paintManager.setOnPaintApplied((paint) => {
            this._onPaintApplied(paint);
        });
        if (options.userData) {
            this.userData = Object.assign({}, JSON.parse(JSON.stringify(options.userData)));
        }
        if (options.paint) {
     ...
     */
    _applyCoordinateChanges(fastUpdate = false) {
    ...
    _disposeGeometry() {
        ...
        WebGPUCompat.safeDispose(() => {
            if ('traverse' in this) {
                this._renderObject.traverse(obj => {
     ...
     */
    getScreenBoundingBox(camera, renderer) {
        if (!this.collidable)
            return null;
        if (!this._renderObject)
            return null;
        try {
            ...
            // 检查是否在屏幕内
            const inFrustum = (screenPos.x >= -1.1 && screenPos.x <= 1.1 &&
    ...
    _calculateCollisionBoundingBox(camera, renderer) {
        ...
        try {
            ...
            // 计算世界空间包围盒
            const bbox = new Box3().setFromObject(this._renderObject);
            if (bbox.isEmpty())
     ...
     */
    // @ts-ignore
    _tileCoordToLocalWorld(rawX, rawY, tileData, map) {
        ...
        return featureLocalWorldPos.sub(map.prjcenter);
    }
}
