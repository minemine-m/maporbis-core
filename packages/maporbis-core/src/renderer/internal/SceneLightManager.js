import { AmbientLight, DirectionalLight, Object3D, CameraHelper } from 'three';
/**
...
export class SceneLightManager {
    ...
    initialize(config) {
        ...
        this._ambientLight = this._createAmbientLight(mergedConfig.ambientColor, mergedConfig.ambientIntensity);
     ...
     * 创建环境光
     */
    _createAmbientLight(color, intensity) {
     ...
     * 创建主平行光
     */
    _createDirectionalLight(config) {
        const x = 1.2;
        const y = 2;
        const z = 1;
        ...
        dirLight.position.set(this._centerWorldPos.x + size * x, size * y, this._centerWorldPos.z + size * z);
     ...
     * 创建辅助平行光
     */
    createAuxiliaryLights(intensity = 0.5) {
        const size = SceneLightManager.DEFAULT_CONFIG.shadowCameraSize;
        // Back fill light
        const auxLight1 = this._createAuxiliaryLightInstance(this._centerWorldPos.x + size * -1.2, size * 1.5, this._centerWorldPos.z + size * -1, intensity, 'AuxDirLight_BackFill');
        // Left rim light
        const auxLight2 = this._createAuxiliaryLightInstance(this._centerWorldPos.x + size * -1.0, size * 1.5, this._centerWorldPos.z + size * 1.2, intensity, 'AuxDirLight_LeftRim');
        // Right rim light
        const auxLight3 = this._createAuxiliaryLightInstance(this._centerWorldPos.x + size * 1.0, size * 1.5, this._centerWorldPos.z + size * -1.2, intensity, 'AuxDirLight_RightRim');
     ...
     * 创建单个辅助光源实例
     */
    _createAuxiliaryLightInstance(x, y, z, intensity, name) {
