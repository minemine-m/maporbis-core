 ...
 * @returns 是否匹配 True if matched, false otherwise
  * @category Style
 */
export function matchFilter(filter, properties) {
    ...
    switch (op) {
        case 'all':
            return filter.slice(1).every((sub) => matchFilter(sub, properties));
        case 'any':
            return filter.slice(1).some((sub) => matchFilter(sub, properties));
        case '!':
        ...
        case 'in': {
            ...
            const rest = filter.slice(2).map((x) => evalExpr(x, properties));
 ...
 */
function evalExpr(expr, properties) {
