...
export class TerrainMeshBuilder {
     ...
     */
    build(terrainData, maxError = 0) {
        const errors = this.calculateErrors(terrainData);
        const rawData = this.generateGeometry(errors, maxError);
        const vertices = rawData._vertices;
        const indices = rawData.indices;
     ...
     * 从误差图生成几何体。
     */
    generateGeometry(errors, maxError) {
        ...
        const countElements = (ax, ay, bx, by, cx, cy) => {
            const mx = (ax + bx) >> 1;
            const my = (ay + by) >> 1;
