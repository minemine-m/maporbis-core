...
export class GeometrySkirtUtils {
     ...
     */
    static addSkirt(geometryData, skirtHeight, externalEdgeIndices) {
     ...
     * @param indices 三角形索引
     */
    static extractBoundaryEdges(indices) {
        const edges = [];
        // 1. 收集所有边
        ...
        const edgeSourceMap = new Map(); // Store original directed edge
     ...
     */
    static getEdgesFromIndices(indices, position) {
        const edges = [];
        // 辅助排序函数
        const sortIndices = (arr, compareFn) => {
        ...
        // 获取位置辅助
        const getX = (i) => position[i * 3];
        const getY = (i) => position[i * 3 + 1];
        // 排序逻辑 (复刻原代码)
        // West: sort by Y asc
