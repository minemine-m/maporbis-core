 ...
 * const name = requireParam(user.name, 'name', '用户名不能为空');
  * @category Utils
 */
export declare function requireParam<T>(value: T | undefined | null, paramName: string, customErrorMsg?: string): T;
/**
 * 对象属性必填校验（支持深层嵌套）
 ...
 * const config = requireProp(settings, 'server.port', '服务器端口配置缺失');
  * @category Utils
 */
export declare function requireProp<T>(obj: Record<string, any>, propPath: string, customErrorMsg?: string): T;
/**
 * @category Utils
 */
export declare function mapValue(value: number, min1: number, max1: number, min2?: number, max2?: number): number;
