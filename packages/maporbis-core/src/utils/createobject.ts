import {
    Vector2, Vector3, BufferGeometry, BufferAttribute, Points, PointsMaterial, SpriteMaterial, Sprite, Color, MeshBasicMaterial, Mesh, BackSide, DoubleSide, FrontSide, ShaderMaterial, RepeatWrapping, TextureLoader, Shape, ShapeGeometry, MeshStandardMaterial, CanvasTexture, MathUtils, NearestFilter, LinearMipmapLinearFilter, Texture, NormalBlending, InstancedMesh, CylinderGeometry, AdditiveBlending, Object3D,
    LinearFilter, SRGBColorSpace, TubeGeometry, CurvePath, LineCurve3, QuadraticBezierCurve3, MeshPhongMaterial, Material, Path, ExtrudeGeometry
} from 'three';
...
import { CirclePaint, LinePaint, IconPaint, FillPaint, ExtrusionPaint, WaterPaint, CloudPaint, TextPaint, SymbolPaint, LightPaint, Paint, FlowTubePaint, ArrowPaint, FlowTexturePaint, WaterPaintUnion, HighlightWaterPaint } from '../style';
import { Cloud as vanillaCloud } from "@pmndrs/vanilla";
import { normalizeAnchor } from '../types';

// Material cache to reduce overhead
const _iconMaterialCache = new Map<string, Promise<SpriteMaterial>>();
 ...
 * @category Utils
 */
export function _createHighlightWater(config: HighlightWaterPaint, _map: ProjectMap, polygons: Vector3[][][]): Mesh { 
    const { geometry, center, avgY } = _createPolygonGeometry(polygons); 
 ...
 */
function calculateSignedArea(ring: Vector3[]): number {
 ...
 * 反向ring的顶点顺序
 */
function reverseRing(ring: Vector3[]): Vector3[] {
...
export function _createPolygonGeometry(
    ...
    polygons.forEach((rings, polyIdx) => {
        ...
        // 转换外环坐标
        const outerPoints = outerRing.map(v => new Vector2(
            v.x - center.x,
            -(v.z - center.z)
        ));
        ...
        for (let i = 1; i < rings.length; i++) {
            ...
            // 转换孔洞坐标
            const holePoints = holeRing.map(v => new Vector2(
                v.x - center.x,
                -(v.z - center.z)
            ));
...
// 路灯 points
export async function createLight(config: LightPaint, geometries: any, map: ProjectMap) {
