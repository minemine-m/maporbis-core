 ...
 * const name = requireParam(user.name, 'name', '用户名不能为空');
  * @category Utils
 */
export function requireParam(value, paramName, customErrorMsg) {
    if (value === undefined || value === null || value === '') {
        throw new Error(customErrorMsg || `Parameter "${paramName}" is required but received: ${value}`);
    }
    return value;
}
/**
 ...
 * const config = requireProp(settings, 'server.port', '服务器端口配置缺失');
  * @category Utils
 */
export function requireProp(obj, propPath, customErrorMsg) {
    const pathSegments = propPath.split('.');
    let current = obj;
    for (const segment of pathSegments) {
        if (current[segment] === undefined || current[segment] === null) {
            throw new Error(customErrorMsg || `Property "${propPath}" is required but missing at path: "${segment}"`);
        }
 ...
 * @category Utils
 */
export function mapValue(value, min1, max1, min2 = 0, max2 = 1) {
    // 处理输入范围为0的情况
    if (min1 === max1) {
        return min2;
    }
